;; Copyright (C) 2005 Darko Marinov - marinov@alum.mit.edu
;; Licensed under the Modified BSD Licence; see COPYING for details.
;;
;; Useful for people doing their development under Emacs
;;
;; Contains useful settings for Emacs jde (e.g., generating the right
;; copyright information at the beginning of each class); also,
;; several emacs macros.
;;
;; Initially written by Darko for the MIT Flex project; minor
;; modifications by Alex Salcianu - salcianu@alum.mit.edu

;;;; extract the package name from the buffer-file-name
(defun myprj-subdir-name (basename-fragment)
...
(defun myprj-basepath-name (basename-fragment)
...
(jde-set-variables 
 ...
 '(jde-gen-class-buffer-template (quote 
    ("\"// \" (file-name-nondirectory buffer-file-name)"
     ...
     "\"// Copyright (C) 2006 \" (user-full-name) \" <\" user-mail-address \">\" 'n" 
     "\"// Licensed under the Modified BSD Licence; see COPYING for details.\" 'n" 
     "\"package jpaul\" (myprj-package-name \"/jpaul\") \";\" 'n 'n" 
     "\"/**\" 'n" 
 ...
 '(jde-gen-code-templates (quote (("Get Set Pair" . jde-gen-get-set) ("toString method" . jde-gen-to-string-method) ("Action Listener" . jde-gen-action-listener) ("Window Listener" . jde-gen-window-listener) ("Mouse Listener" . jde-gen-mouse-listener) ("Mouse Motion Listener" . jde-gen-mouse-motion-listener) ("Inner Class" . jde-gen-inner-class)  ("Visitor Class" . tempo-template-myprj-visitor-class)))))
